...
import { buildVisualQueryFromFacts, parseSceneFacts } from "./sceneVisualParser";

const SCENE_COLORS = ["#1f2937", "#0f766e", "#7c2d12", "#1d4ed8", "#6d28d9"];

export async function generateSceneVisuals(sceneText: string, outputDir: string, sceneId: number) {
    const cleanedSceneText = normalizeSceneText(sceneText);
    const shotCount = pickShotCount(cleanedSceneText);
    const parsedFacts = parseSceneFacts(cleanedSceneText);
    const queryPlan = buildImageQueryPlan(parsedFacts, shotCount);
    const outputs: string[] = [];

    for (let i = 0; i < queryPlan.length; i++) {
        const outputPath = path.join(outputDir, `scene-${sceneId}-shot-${i + 1}.jpg`);
        const downloaded = await tryDownloadSceneImage(queryPlan[i], outputPath);
        if (downloaded) outputs.push(outputPath);
    }

    if (outputs.length > 0) return outputs;

    // Fallback when image API is unavailable.
    const fallbackOutputPath = path.join(outputDir, `scene-${sceneId}-visual.png`);
    ...
    fs.writeFileSync(captionPath, wrappedText, "utf-8");
...
function wrapTextForScene(text: string, maxCharsPerLine = 42, maxLines = 8) {
...
async function tryDownloadSceneImage(query: string, outputPath: string): Promise<boolean> {
    ...
    const searchJson = (await searchResponse.json()) as {
        photos?: Array<{ src?: { landscape?: string; large2x?: string; large?: string }, alt?: string }>;
    };
...
function buildImageQueryPlan(facts: ReturnType<typeof parseSceneFacts>, shotCount: number) {
    const base = buildVisualQueryFromFacts(facts);
